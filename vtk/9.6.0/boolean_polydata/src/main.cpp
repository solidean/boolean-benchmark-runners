// ---------------------------------------------------------------------------
// main.cpp — vtk_boolean_runner entry point
//
// Execution contract:
//   vtk_boolean_runner --request <request.json> --result <result.json>
//
// Uses vtkBooleanOperationPolyDataFilter for union, intersection, and
...
// Attach this observer to a filter before Update() to catch VTK errors.
struct VtkErrorFlag
{
                                ...
                                void* client_data, void* /*call_data*/)
{
    ...
    flag->triggered = true;
    // vtkOutputWindowDisplayErrorText provides the message via call_data as char*,
    // but vtkCallbackCommand doesn't expose it here; the error was already printed.
    flag->message = "vtkBooleanOperationPolyDataFilter reported an error";
}
...
// Build a vtkPolyData from flat indexed-tris-f64 and run CleanPolyData +
// TriangleFilter.  Returns nullptr if the result has 0 points.
...
// Extract a vtkPolyData to flat indexed-tris-f64.
...
// Run vtkBooleanOperationPolyDataFilter.  Returns {result, ok}.
// ok==false means the filter reported a VTK error.
...
apply_op(vtkPolyData* lhs, vtkPolyData* rhs, OpKind kind)
{
    ...
    // Attach error observer before Update().
...
static json execute_run(runner_utils::Config const& /*cfg*/, json const& run_req)
{
    ...
    try
    {
        ...
        // SSA storage: ssa[i] = preprocessed vtkPolyData result of op i.
        ...
        for (std::size_t i = 0; i < ops.size(); ++i)
        {
            ...
            try
            {
                if (op_str == "load-mesh")
                {
                    ...
                    // import phase — flat f64 → vtkPolyData + clean + triangulate
                    Timer import_timer;
                ...
                {
                    validate_op_boolean_binary(i, ops, "vtk_boolean_runner");
...
{
    runner_utils::Config const cfg = runner_utils::parse_args(argc, argv, "vtk_boolean_runner");
